...
As a taster of what we will do, and to check you have the right input data, the results should look something like the map shown in Figure \@ref(fig:accesso).
...
<!-- It does not account for whether the infrastructure actually goes where people want to go. -->
...
- Search for and select 'Distance from nearest hub (points)' in the Processing Toolbox (see Figure \@ref(fig:processing-hub-distance), left)
...
- List at least 2 disadvantages of this new accessibility measure and 2 ways you could improve it using your evolving QGIS skills.
...
Although the desire lines represent travel, they clearly are not the routes that people actually take from home to work (the dataset comes from the 2011 Census question on travel to work).
...
- Bonus 1: Download the fast and quiet route data from the PCT's Region Data tab and load it into QGIS (see top of Figure \@ref(fig:pct-west) for the Region data tab).

- Bonus 2: is there positive correlation between cycling levels and the length of cycle infrustructure in the buffer of each desire line?
