...
QGIS allows you to open multiple files at once and overlay them as layers on the same map. You can also customise the visualisation of the layers. To allow you to save your work it is recommended to save a project file.  Projects don't contain any data themselves, but do contain information such as the current map view, links to data, and instructions on how data should be presented. Projects are an easy way to keep your work organised and allows you to stop and come back to work at a later date.

You can create a project and save a project using the buttons shown below or using the options in the Project menu.


```{r probect_buttons, fig.cap="New Project, Open Project, Save Project, Save Project As buttons", out.width="50%"}
knitr::include_graphics("figures/project_buttons.jpg")
```

Note that because the Project does not contain any data itself you cannot simple move a project file from one computer to another. It would be necessary to move both the project and any associated files. You may also need to redirect the project to the new file locations on the new computer.

## Projections and Cordinate Refercne Systems

```{r projections, fig.cap="Map Projections, Source XKCD", out.width="50%", fig.align = "right"}
knitr::include_graphics("figures/map_projections.png")
```
...
Cartographers can argue intensely about their preferred projections as this famous XKCD comic alludes to. Coordinate reference systems refers to different ways of defining the X and Y coordinates used in different projections. Largely they fall into two categories:
